...
const CACHE_CONTENTTYPE_LIST = ['image/png', 'image/gif', 'image/jpeg', 'application/font-woff'];
      ...
      .then(function (cacheList) {
        ...
        return Promise.all(
          cacheList.map(function (cacheNameVersion) {
            var cacheName = /^[^\d.*]*/gi.exec(cacheNameVersion); // 버전관리를 위한 서비스워커 저장 캐시 Name 확인
            console.log('[Service Worker] cacheName', cacheName);
            if (cacheName && cacheName[0] === CACHE_NAME_STATIC && cacheWhitelist.indexOf(cacheNameVersion) === -1) {
              // 현재 버전이 아닌 과거버전 확인
              // 기존버전 제거
  ...
  event.respondWith(
    caches.match(event.request).then(function (response) {
      ...
      if (event.request.method !== 'GET' || NETWORK_LIST.indexOf(requestURL.pathname) !== -1) {
        ...
        return fetch(event.request).catch(setFetchCatch);
      } else if (response) {
        // 2. 캐시 데이터 반환
        ...
        return fetch(fetchRequest)
          .then(function (response) {
            ...
            if (
              ...
              response.status >= 400 ||
              response.type === 'error' ||
              (event.request.mode === 'same-origin' && response.type === 'cors') ||
              (event.request.mode !== 'no-cors' && response.type === 'opaque') ||
              (event.request.redirect !== 'manual' && response.type === 'opaqueredirect')
            ) {
              ...
              */
              ...
              responseToCache = response.clone();
              if (
                CACHE_CONTENTTYPE_LIST.indexOf(response.headers.get('content-type')) !== -1 ||
                event.request.url.indexOf('external.url') !== -1
              ) {
                caches.open(CACHE_NAME_STATIC).then(cache => {
                  console.log(
                    `[Service Worker] 네트워크 요청 데이터 캐시에 추가 (${CACHE_NAME_STATIC + CACHE_VERSION})`,
                    responseToCache,
                  );
                  cache.put(event.request, responseToCache);
                });
...
self.addEventListener('sync', function (event) {
  ...
  // sendToServer
  switch (
    event.tag // event.tag 는 클라이언트에서 이벤트 설정
  ) {
