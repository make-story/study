...
const DATETIME = (() => {
  ...
  return [[value.year, value.month, value.day].join(''), [value.hour].join('')].join('-');
})();
...
const CACHE_VERSION = '20230309';
const CACHE_NAME_SCRIPT = [CACHE_NAME, CACHE_VERSION, 'SCRIPT'].join('-');
const CACHE_NAME_STYLE = [CACHE_NAME, CACHE_VERSION, 'STYLE'].join('-');
const CACHE_NAME_FONT = [CACHE_NAME, CACHE_VERSION, 'FONT'].join('-');
const CACHE_NAME_IMAGE = [CACHE_NAME, CACHE_VERSION, 'IMAGE'].join('-');
const CACHE_NAME_IMAGE_CDN_BE = [CACHE_NAME, CACHE_VERSION, 'IMAGE', 'CDN', 'BE'].join('-'); // 백엔드 CDN 관리 리소스
const CACHE_NAME_IMAGE_CDN_FE = [CACHE_NAME, CACHE_VERSION, 'IMAGE', 'CDN', 'FE'].join('-'); // 프론트 CDN 관리 리소스

/**
 * 서비스워커 실행
 */
const setServiceWorkerRun = () => {
  /**
   * install
   */
            ...
            ...cacheList.map(function (cacheName) {
              ...
              if (
                ![
                  ...
                  CACHE_NAME_STYLE,
                  CACHE_NAME_FONT,
                  CACHE_NAME_IMAGE_CDN_BE,
                  CACHE_NAME_IMAGE_CDN_FE,
                ].includes(cacheName)
              ) {
...
const setWorkBoxRun = workbox => {
  ...
  const isContext = context =>
    isObject(context) && isObject(context.url) && isObject(context.request) && isObject(context.event)
      ? context
      : false;
  // MIME 유형 확인 (Content-Type응답 헤더)
  const isAccept = (context, mime = '') =>
    isContext(context) && context.request.headers ? context.request.headers.get('accept').includes(mime) : false;
  // hostname 확인 - hostname 은 host 에서 포트번호를 제거한 부분 - 예: local.amoremall.com
  const isHostname = (context, hostname = []) =>
    isContext(context) && context.url.hostname ? hostname.includes(context.url.hostname) : false;
  // origin 확인 - 예: https://local.amoremall.com
  const isOrigin = (context, origin = '') =>
    isContext(context) && context.url.origin ? context.url.origin.includes(origin) : false;
  // pathname 확인 - 예: /dsp-stg-upload-amt/fileupload/display/AP01/CTG010001/ko/2021/07/13/icon_woman@3x.png
  const isPathname = (context, pathname = '') =>
    isContext(context) && context.url.pathname ? context.url.pathname.includes(pathname) : false;
  // 콘텐츠 유형 확인
  const isDestination = (context, destination = []) =>
    isContext(context) && context.request.destination ? destination.includes(context.request.destination) : false;
  // 확장자 확인
  const isExtension = (context, extension = []) =>
    isContext(context) && context.request.url && Array.isArray(extension)
      ? new RegExp(`.*\\.(?:${extension.join('|')})([\\?|#].*)?$`).test(context.request.url)
      : false;
   ...
   */
  workbox.setConfig({
    //debug: ['local.amoremall.com', 'localhost'].includes(self.location.hostname),
  ...
  workbox.routing.registerRoute(
    ...
    new workbox.strategies.StaleWhileRevalidate({
      ...
      plugins: [
        ...
        new workbox.expiration.ExpirationPlugin({
          //maxEntries: 10, // 캐시 할 최대 리소스 수
          maxAgeSeconds: 1 * 24 * 60 * 60, // 1 일 - 86400 초 (씨디네트웍스 CDN 캐시설정 기본값은 max-age=604800, 그러나 오리진에서 직접 Cache-Control 헤더를 설정하면 그 헤더 설정을 따르게 됨)
          //maxAgeSeconds: 1 * 60 * 60, // 1 시간 - 3600 초
          //maxAgeSeconds: 5 * 60, // 5 분
        }),
      ],
      fetchOptions: {},
      matchOptions: {},
