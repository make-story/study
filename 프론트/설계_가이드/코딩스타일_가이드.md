...
1. `코딩스타일 가이드 필요성 대한, 공감대 형성`
2. `외부 가이드 자료 수집 및 우리 코딩 스타일과 유사(코딩 스타일)하거나 코딩 스타일 가이드 수립에 필요한 항목 리스트업(우리 가이드 기반자료)`
3. `코딩 가이드 초안 작성`
4. `협의체 회의 진행하여 코딩가이드 초안 재정립`
5. `코드 리뷰 기반자료로 활용하며, 지속적 가이드 업데이트(단, 업데이트는 구성원 모두의 동의된 것에 한함)`
...
`컴포넌트 내 폴더(components 폴더) 구조는 기본 아토믹 디자인 패턴을 따르며, pages 폴더는 Next.js 프레임워크 구조에 따른 해당 경로를 사용한다`    
