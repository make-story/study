...
- 리소스
  - 썸네일
  - 반응형 이미지 (picture tag 활용 화면밀도에 맞는 이미지 대응)
  - 레이지 로드 (지연로드)
  - 파일 포맷 (WebP)
  - Gif > Video (Video 최적화 기술 활용)
  - HTTP 캐시컨트롤, 서비스워커 (캐시 전략 활용)
- 코드
  - 리플로우, 리페인트 (DocumentFragment 표준 WebAPI 활용)
  - 리렌더 방지 (useCallback, React.memo)
  - preload 속성
  - Img tag 속성 (importance, loading)
  - script load (async, defer)
  - 다이나믹(동적) script file import
- 도구
  - 트리쉐이킹 (필요한 코드만 import)
  - 번들 (하나의 파일로 합치기)
  - 청크 (코드 분리된 파일)
- 인프라
  - 바니시캐시 소프트웨어
...
CJ ENM - 서버사이드 렌더링, 바니시 캐시서버(앳지서버)
...
`하단 영역 컨텐츠 레이지로드 (네트워크 우선순위를 상단에 할당)`
...
## 반응형 이미지 WEB API (웹표준 방식) - 화면밀도 대응 (썸네일)
...
- 최신 Web 표준 API Img importance, loading 활용
- preload
...
- DocumentFragment
  ...
  - Document 의 경량화된 버전
  - Fragment 에서의 변경사항은 원본 문서에 리플로우/리페인트 등의 영향을 주지 않음

- 자바스크립트 배치

  - HTML 파싱 단계에서의 자바스크립트 비동기 로드
  - defer/async, document.write()
...
- 메모이제이션, 웹워커
  - 여러 연산작업을 동시에 해야할 경우 웹워커 사용

# 4. 인프라 (네트워크, 캐시 등)

- HTTP Cache-Control

- CDN, 캐시 앳지서버 (바니시 캐시 등)
...
## Next.js dynamic import (청크파일 분리)
