...
## 루트 디렉토리에서 packages 디렉토리 생성 후, 그 안에 pack-a 와 pack-b 패키지를 생성 (root 폴더 하위 여러 프로젝트 단위 폴더 생성)
...
## 개별 프로젝트 코드에서 상위(공통, common 등) 프로젝트 코드 import 사용 예
pack-a/Button.js
```javascript
...
import { Button } from "@monorepo/pack-a/Button";
const Home = () => (
