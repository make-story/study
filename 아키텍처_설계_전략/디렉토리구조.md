...
https://twitter.com/mjovanovictech/status/1769634553012928998  
...
위 3가지를 손쉽게 구축/확장/공유할 수 있도록 해주는 것이 모노레포
...
```
helper/performance.ts
```

위와 같이 performance.ts 파일 하나에 관련 기능(함수)들을 선언하고 관리하다가  
기능이 지속 추가 또는 분리가 필요한 시점에  
배럴파일(Index, 단일 파일에서 여러 모듈을 그룹화하고 내보내는 방법) 방식활용

```
helper/performance/index.ts
```

위와 같이 할 경우,  
기존의 import {} from 'helper/performance' 코드 수정없이 확장 가능
