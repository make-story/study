...
`https://github.com/make-story/nextjs-monorepo-example`
...
젠킨스에서는  
배포하려는 `프로젝트 (apps/mall)`  
연관된 `패키지 (packages/*)`  
`즉, 배포에 필요한 파일만 가져와서 배포. 저장소 전체코드가 아닌 실제 배포가 필요한 애플리케이션(예: apps/mall) 코드만 가져와 빌드`

모노레포의 공통 패키지는 `심볼릭링크로 연결되므로 빌드결과 압축이 필요한 경우, Zip 파일로 압축!`  
Jenkins 설정 > Archive for Clone Workspace SCM > Archive method > Zipped 설정
...
`study.git/인프라/서버/NodeJS_NPM/Yarn.md` 참고!
...
`Next.js 버전에 따라, next-transpile-modules 지정된 버전(Next.js 버전에 맞는 패키지 버전, Compatibility table) 사용해야 함`  
...
## 모든 패키지가 yarn workspaces 의 호이스팅 방식을 지원하는 것은 아님 (즉, 모든 npm 모듈들이 모노레포를 지원하는 것이 아님)

https://classic.yarnpkg.com/blog/2018/02/15/nohoist/

https://github.com/connectdotz/yarn-nohoist-examples  
https://github.com/connectdotz/yarn-nohoist-examples/blob/master/workspaces-examples/universal-cipher/package.json

루트 수준(모노레포에서 최상위 node_modules 폴더)에 있는 모든 종속성을 끌어올리는 것(하위 모노레포 패키지에서 최상위 node_modules 를 바라보는 것)을 중지
...
{
  "workspaces": {
    "packages": ["apps/*", "micro/*", "packages/*"],
    "nohoist": ["**/nextjs12/**", "**/nextjs13/**"] // apps 하위 'nextjs12', 'nextjs13' 설정
  }
}
```

`패키지(프로젝트)에서 사용하는 모듈들 모두 node_modules 에 정상 설치 되었음에도 불구하고 import 에 빨간줄이 나오는 경우, VSCode 재시작 권장`

```json
{
  "workspaces": {
    "packages": ["apps/*", "micro/*", "packages/*"],
    "nohoist": ["nextjs12/**", "**/nextjs12/**"] // 다른 사이트 설명 참고
  }
}
```

https://classic.yarnpkg.com/blog/2018/02/15/nohoist/  
`중요! nohoist 는 yarn 1.4.2 이상 지원!`
...
# 중요: 각 프로젝트(패키지 또는 애플리케이션)별 Node.js 버전이 다른 경우

- 최상위 package.json 의 workspaces 항목에서 버전이 다른 프로젝트(패키지 또는 애플리케이션) 제외
- 제외한 상태에서 해당 프로젝트 폴더로 이동한 다음 패키지 설치($ yarn install)
- 이후 최상위 package.json 의 workspaces 원복
...
1. mall 프로젝트, community 프로젝트에서 공통으로 사용하는 코드(의존성 모듈)를 관리/공유하려면? (각각의 프로젝트에서 사용하는 공통코드 공유)

   - mall 프로젝트 내부에 있는 `common 코드를 NPM 저장소에 배포, community 프로젝트에서 NPM 설치`하여 사용?
   ...
   - mall 프로젝트 `저장소를 Fork 하는 방식(mall 저장소 업스트림으로 그 외 저장소들은 다운스트림)`으로 하여, 업스트림에 있는 common 코드를 다운스트림에서 풀 받고, 다운스트림에서 수정하는 경우, 업스트림(MR)으로 관리한다.
     ...
     - 이 경우, 공통 코드 수정 영향 범위가 프로젝트 어느 범위(각각의 저장소에 들어가 코드 검색 필요)까지 인지 찾기가 힘들다.

2. mall 프로젝트 저장소, community 프로젝트 저장소를 하나의 저장소에서 관리하려면? (각각의 프로젝트를 하나의 저장소에서 관리)

   - project 저장소를 만들고, `mall, community 폴더를 각각 만들어 관리`한다. - 단순 디렉토리 방식으로 구조화하여 관리
     - 이 경우, 각 프로젝트에서 중복되는 node module 은 각각의 프로젝트에 중복설치 발생 (뭐 이런 것 기준에도 신경쓰지 않았으니 그냥 한다?)

3. 단순하게 가자?
   project.git > common 폴더, mall 폴더, community 폴더, ... 폴더
   폴더(디렉토리) 구조로?
   - 각 프로젝트에서 공통 사용하는 node module 중복 설치함
   - jenkins 배포시(예: mall 프로젝트) common, mall 폴더 받아와서 빌드?

공통으로 사용하는 라이브러리에서 인터페이스가 변경되는 Breaking Change가 발생하면, 의존하고 있는 모든 서비스의 코드를 수정  
