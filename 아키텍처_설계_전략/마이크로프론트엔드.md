...
## NPM 패키지 교환방식의 마이크로 프론트엔드 방식의 단점, 이를 해결하기 위한 Module Federation 구축

https://www.nextree.io/module-federation/

수정한 NPM 패키지 배포, 이를 의존하는 프로젝트 또는 패키지 배포  
즉, 배포가 두번 또는 프로젝트 크기에 따라 더 많은 배포를 해야할 수 있음

Module Federation 은 배포 단위를 페이지 단위 이하로, 디자인시스템 단위 배포가 가능하다는 점
...
- `자바스크립트를 통한 런타임 통합(Run-time integration via Javascript) - @module-federation`
...
흥미롭게도 마이크로 프론트엔드의 초창기에는 대부분 사람들이 기술적인 분할에 열광했다.  
그러나 `마이크로 프론트엔드의 진정한 이점은 비즈니스 중심의 분할 결정에서 나타난다.`
